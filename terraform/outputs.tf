...
output "web_public_ip" {
  ...
  value       = aws_instance.devops_web[0].public_ip

  // This output waits for the Elastic IPs to be created and distributed
  depends_on = [aws_instance.devops_web]
}
...
output "web_public_dns" {
  ...
  // We are grabbing it from the Elastic IP
  value       = aws_instance.devops_web[0].public_dns
...
#   value       = aws_db_instance.devops_database.address
# }

# // This will output the database port
# output "database_port" {
#   description = "The port of the database"
#   value       = aws_db_instance.devops_database.port
# }